...
export function PricingTable({
	...
	const hasActiveSubscription =
		activePlanId && activePlanId !== "free" && activePurchaseId;
	...
	const onSelectPlan = async (planId: PlanId, productId?: string) => {
		...
		try {
			// 如果用户已有订阅，跳转到Customer Portal进行升降级
			...
			// 新用户：创建Checkout Session
			const redirectUrl = new URL(window.location.href);
			redirectUrl.searchParams.set("checkout_success", "true");
	...
	// 过滤掉free套餐（如果用户已有订阅），但保留当前套餐以便显示"当前套餐"状态
	const filteredPlans = Object.entries(plans).filter(
		([planId]) => !activePlanId || planId !== "free",
	);
		...
		<div className={cn("@container", className)}>
			...
			>
				{filteredPlans.map(([planId, plan]) => {
					...
					const { title } = planData[planId as keyof typeof planData];
					...
					// 判断是否为降级
					const currentPlanLevel = activePlanId
						? getPlanLevel(activePlanId)
						: 0;
					const targetPlanLevel = getPlanLevel(planId);
					...
					return (
						...
						>
							{isCurrentPlan ? (
								<div className="absolute -top-4 left-0 right-0 mx-auto w-fit rounded-full bg-green-600 px-4 py-1.5 text-xs font-bold text-white shadow-sm ring-4 ring-background">
							...
							<div className="flex flex-col pb-8">
								...
								<div className="mt-6">
									{isEnterprise ? (
										...
										<Button
											className="w-full rounded-xl font-bold h-12 bg-muted text-muted-foreground cursor-not-allowed"
											variant="outline"
											size="lg"
											disabled
										>
